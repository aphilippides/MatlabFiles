...
for i=1:NumEnvs
    ...
    for r=ObjR
        ...
        if(Drawing)
            ...
            % MyCircle(NestPos(1),NestPos(2),NestR,20)
            hold on;
            MyCircle(FoodPos,FoodR,'r')
            % MyCircle(FoodPos(1),FoodPos(2),FoodR,20)
            for j=1:NumObj
                          MyCircle(ObjectPos(j,:),ObjectRs(j))
                %MyCircle(ObjectPos(j,1),ObjectPos(j,2),ObjectRs(j),20)
            end
            hold off
            pause
        end
        ...
        if(Printing)
            fn = ['Env' int2str(i) 'Trial' int2str(r) '.env'];
            fn = ['Env' int2str(i) 'Trial' int2str(r) '.mat'];
            fid=fopen(fn,'w');
