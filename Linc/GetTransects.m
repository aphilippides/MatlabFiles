function GetTransects(startpt,n,m)
dwork
...
while(1)
    s=LincVision_2(os(:,[1 2]),os(:,3),size(os,1),startpt);
    angs=s*pi/45;
    snapx=mean(cos(angs));
    snapy=mean(sin(angs));
    [obj,mx,my]=getPics(X,Y,agentrad,os,startpt,snapx,snapy);
    figure(1)
...
% [X,Y] = meshgrid([0:283],150);
% [X,Y] = meshgrid(88:0.5:108,21:0.5:41);
[X,Y] = meshgrid(40:0.5:60,90:0.5:110);
% [X,Y] = meshgrid(52,[0:170]);
[obj,mx,my]=getPics(X,Y,agentrad,os,startpt,snapx,snapy,0);
d=diff([mx' my'])
% d=[mx' my']
s=sqrt(sum(d.^2,2));
s=sqrt(mx.^2+my.^2);
...
dwork;
cd ../Current/Linc/
pcolor(X,Y,double(s>=0.02)+double(s>=0.05))
hold on, plot(50,100,'wo'), hold off
% hold on, plot(100,30,'wo'), hold off
colormap gray,shading flat,axis equal,axis tight

print -djpeg -r900 CurrentMinusGoal_X50Y100Thresh.jpg
% print -djpeg -r900 CurrentMinusGoal_X100Y30Thresh.jpg
figure(10)
pcolor(X,Y,s),colorbar,shading flat,axis equal,axis tight
hold on, plot(50,100,'wo'), hold off
% print -djpeg -r900 CurrentMinusGoal_X100Y30withCentre.jpg
print -djpeg -r900 CurrentMinusGoal_X50Y100withCentre.jpg
plot(s), axis tight
subplot(2,1,2)
plot(diff(obj)~=0), axis tight
keyboard
% 
...
function[NObj,mx,my]=getPics(X,Y,agentrad,O,startpt,snapx,snapy,drawing)

if(nargin<8) drawing=1; end;
for i=1:size(X,1)
    for j=1:size(X,2)
        ...
        if(InsideObject(O,agent))
            ...
            s=LincVision_2(O(:,[1 2]),O(:,3),size(O,1),agent([1 2]));
            NObj(i,j)=length(s);
            if(NObj(i,j)==0)
                mx(i,j)=NaN;
                my(i,j)=NaN;
