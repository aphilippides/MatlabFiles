function SolPicsTest(Ts,Vers,MaxW,DirNum,Tstep,Thresh,MinW,dstr)
cmin=0;
cmax=0;
dmat; 
...
% for t=[40:6400:65000]
    tbmp=t/Tstep;
    if(tbmp<10) fn=['K:\ScratchAll\lincolns\sols\image' int2str(DirNum) '\' nstr ' 00' int2str(tbmp) '.bmp']
    elseif(tbmp<100) fn=['K:\ScratchAll\lincolns\sols\image' int2str(DirNum) '\' nstr ' 0' int2str(tbmp) '.bmp']
    else fn=['K:\ScratchAll\lincolns\sols\image' int2str(DirNum) '\' nstr ' ' int2str(tbmp) '.bmp']
    end
    [i,cmap]=imread(fn);
