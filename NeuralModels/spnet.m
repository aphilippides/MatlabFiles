% Spiking network with axonal delays and STDP
...
for i=1:N
  ...
  aux{i}=N*(D-1-ceil(ceil(pre{i}/N)/(M/D)))...
         +1+mod(pre{i}-1,N);
end;
STDP = zeros(N,1001+D);
...
for sec=1:60*60*24       % simulation of 1 day
  for t=1:1000           % simulation of 1 sec
    ...
    for k=1:length(fired)
      sd(pre{fired(k)})=sd(pre{fired(k)})...
                        +STDP(N*t+aux{fired(k)});
    end;
    ...
    while firings(k,1)>t-D
      ...
      sd(firings(k,2),del)=sd(firings(k,2),del)...
                           -1.2*STDP(ind,t+D)';
      k=k-1;
    end;
