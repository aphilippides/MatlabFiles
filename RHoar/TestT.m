% function TestT(l1,l2,l3,tvar,tsT,sT,buy,sell,TLength,TOv)
function TestT(fin,TLength,TOv)

[tim,l1,l2,l3,buy,sell,vol,tvar,BV,AV]=ReadLogDataVol(fin);

% load(fin);
iv=1:length(l1);
plot(iv,l1,'g',iv,l2,'g')
hold on;
plot(iv,l3,'r',iv,tvar,'y','LineWidth',2);% ,tsT,sT,'c'
[t1t2,tProf1,tProf2,NumTradesT]=GetTProf(tvar,l3,iv,buy,sell,TLength,TOv);

for k=1:size(t1t2,1)
    ind=t1t2(k,1);
    if(t1t2(k,2)==1) plot([ind ind],[l3(ind)-0.125 l3(ind)+0.125],'b--*','Linewidth',1);
    elseif(t1t2(k,2)==0) plot([ind ind],[l3(ind)-0.125 l3(ind)+0.125],'r--*','Linewidth',1);
