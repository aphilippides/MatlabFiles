...
%    if(1)       % only in for winmerge purposes
        ...
        ivec=xl:i;
        l1p=l1(ivec);l1p(DontPlot)=NaN;
        l2p=l2(ivec);l2p(DontPlot)=NaN;
        ma=max([l1p l2p])+0.02;
        mi=min([l1p l2p])-0.02;
        if(isnan(ma)||((ma-mi)>2))
            ma=l3(i)+1;
        ...
        plot(ivec,l1p,'g',ivec,l2p,'g','Linewidth',2)% ,ivec,l3(ivec),'g'
        hold on
        ...
        l1p=l1(ivec);l1p(DontPlot)=NaN;
        l2p=l2(ivec);l2p(DontPlot)=NaN;
        l3p=l3(ivec);l3p(DontPlot)=NaN;
        ma2=max([l1p l2p])+0.02;
        mi2=min([l1p l2p])-0.02;
        if(isnan(ma2)||((ma2-mi2)>2))
            ma2=l3(i)+1;
            mi2=l3(i)-1;
        end
        nup=ma2+abs(0.02*(ma2-mi2));
        plot(ivec,l1p,'b',ivec,l2p,'r',ivec,l3p,'m','Linewidth',2)
        hold on
        if(NotStillClosed)
