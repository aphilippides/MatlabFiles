function PlotP1P2Lines(i,l1line,l2line,exl1,exl2,Hi2Day,Low1Day,...
...
if(~isempty(HLinesL1)) 
    ...
    text(i+n+addsbuy(end),HLinesL1(end,2),int2str(round(HLinesL1(end,3))),...
        'Color','b','FontSize',HLFsize)
    if(nhl1>1)
        plot([HLinesL1(end-1,1) i],[HLinesL1(end-1,2) HLinesL1(end-1,2)],'b','LineWidth',2);
        text(i+n+addsbuy(end-1),HLinesL1(end-1,2),...
...
if(~isempty(HLinesL2)) 
    ...
    text(i+n+addsell(end),HLinesL2(end,2),int2str(round(HLinesL2(end,3))),...
        'Color','r','FontSize',HLFsize)
    if(nhl2>1)
        plot([HLinesL2(end-1,1) i],[HLinesL2(end-1,2) HLinesL2(end-1,2)],'r','LineWidth',2);
        text(i+n+addsell(end-1),HLinesL2(end-1,2),...
