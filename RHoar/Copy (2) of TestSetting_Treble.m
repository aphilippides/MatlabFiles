...
for i=1:length(L1)%N;
    ...
    if(t(i)>=TimeSecs([16,29,59]))
        ...
        [a,b]=GetProfit(x1x2);
        pr1=sum(a);pr2=sum(b);
        xlabel(ax3,['DAILY CHART:   E Trend Profit A = ' num2str(pr1) '; E Trend Profit B = ' num2str(pr2)]);
...
%         its=(max(1,i-nS2)):i;
        plot(ivec,tvar(ivec),'y','LineWidth',2)
...
%         udp2=sum(GetProfit(updown2));
    ...
    DiagL1,DiagL2,exDiag1,exDiag2,DLinesL1,DLinesL2,HLinesL1,HLinesL2,0.5*spread)        
        ...
        for k=1:size(e1e2,1)
            ind=e1e2(k,1);
            if(e1e2(k,2)==1) plot([ind ind],[l3(ind)-0.25 l3(ind)+0.25],'b-o','MarkerFaceColor','b','LineWidth',2);
            else plot([ind ind],[l3(ind)-0.25 l3(ind)+0.25],'r-o','MarkerFaceColor','r','LineWidth',2);
            end
        end
        for k=1:size(s1s2,1)
            ind=s1s2(k,1);
        ...
        pr1=sum(a);pr2=sum(b);
        xlabel(['DAILY CHART:   E Trend Profit A = ' num2str(pr1) '; E Trend Profit B = ' num2str(pr2)]);
        axis([xl-1 i mi2 ma2])
...
title(['BS = ' num2str(bs1) ';   UD = ' num2str(up1) ]);%';   UD 2 = ' num2str(ud1_2) ]);
hold off;
