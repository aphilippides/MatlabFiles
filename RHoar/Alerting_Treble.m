...
%     nSUD,EPUD,RPUD,TimeLim,p1p2Lim,RepTime)%,nSUD2,EPUD2,RPUD2,nST,TOv)
UpDownOn=0;
BuySellOn=1;
FastBSOn=0;
LineBSOn=1;
...
TimeLim=1000;
p1p2Lim=100;
% Set BS Fast parameters: EPF must be less than nsF
nSF=120;
EPF=0;
RPF=60;
% Old T params: nST=50; % TOv=0.01; % TLength=nST;
...
if(~isfile(fout2)) WriteExcelDataPreambleVol(fout2,2); end;
...
nSUD2=2*nS2UD2;
LastUD2=-3;
% BS Fast: ensure nSF is even and that EPF <=nSF
nSF2=ceil(nSF/2);
nSF=2*nSF2;
EPF=min(EPF,nSF);
...
oBox=[];FirstHors=0;NextETD=[];NextETUp=[];

% fast buy/sell start values
fastbs=[];fBuyOn=0;fSellOn=0;fbsline.xs=[];isF=[];

is=[]; iL1s=[]; iUD2=[]; iTs=[]; DontPlot=[];
for i=1:N
    ts=GetSecs;
    l1(i)=L1;
    l2(i)=L2;
    ...
    if(t(i)>=TimeSecs([16,29,59]))
        WriteExcelDataVol(timez(i,(4:6)),l1(i),l2(i),l3(i),buy(i),sell(i),vol(i),fout2,tvar(i),bv(i),av(i));
    ...
    if((spread>0.01)&(spread<0.6))
        ...
        [ms,MLines]=CheckMids(MLines,l1(i),l2(i),i,buy(i),sell(i),...
            lbuy,lsell,fout,timez(i,(4:6)),LineBSOn);
        mids=[mids;ms];
        ...
        % fast buy sell
        if(FastBSOn)
            % get indices for ns and do the check
            if(length(isF)==(nSF+1)) 
                isF=[isF(2:end) i];
                [fbs,fbsline,fBuyOn,fSellOn]=FastBuySell(l1(isF),l2(isF),l3(isF),isF,...
                    fBuyOn,fSellOn,EPF,RPF,i,...
                    buy(i),sell(i),buysound,sellsound,fout,timez(i,(4:6)));
                fastbs=[fastbs;fbs];                
            else isF=[isF i];
            end
        end
        ...
        if(td>TimeLim)
            if(p1Off&(l2(i)>=exl1))
                ...
                % CHANGED: pt1 meeting 22/9/2010, replaced cash with future values
...
%                 newl=[svalt+svt-1 sval bv(svalt+svt-1)];
        ...
        % get the next possible etup midline
        [NextETUp,NextETD]=GetNextPossLine(l1line,l2line,LastLine,l1,l2,buy,sell,i,Fib);
    ...
    % graph the data
%    if(1)       % only in for winmerge purposes

        % 1 Hourly plot
        subplot(ax2);
        xl=max(1,i-PtsToPlot+1);
        ivec=xl:i;
        tp=setdiff(ivec,DontPlot);
        l1p=l1(tp);
        l2p=l2(tp);
        ma=max([l1p l2p])+0.02;
        mi=min([l1p l2p])-0.02;
        ...
        % fast buy sell
        for k=1:size(fastbs,1)
            ind=fastbs(k,1);
            if(fastbs(k,2)) plot([ind ind],[l3(ind)-0.25 l3(ind)+0.25]);
            else  plot([ind ind],[l3(ind)-0.25 l3(ind)+0.25],'r');
            end
        end
        
        if(NotStillClosed)
            PlotP1P2LinesG1(i,MLines,BLines,fbsline,i-xl,NextETUp,NextETD)
        end        
        ...
        l1p=l1(tp);
        l2p=l2(tp);
        ma2=max([l1p l2p])+0.02;
        mi2=min([l1p l2p])-0.02;
        if(isempty(tp))
            ma2=l3(i)+1;
            mi2=l3(i)-1;
        elseif((ma2-mi2)>2) mi2=ma2-2;
        end
        ...
        if(NotStillClosed)
            PlotP1P2LinesMid(i,l1line,l2line,exl1,exl2,Hi2Day,Low1Day, ...
                ...
                HLinesL1,HLinesL2,MLines,BLines,oBox,0.5*spread,(i-xl),1,xl,av(i),bv(i))
        end
...
is=setdiff([1:length(l1)],DontPlot);
ma2=max(l1(is))+0.02;
mi2=min(l2(is))-0.02;
% if((ma2-mi2)>4)
%     ma2=median(l3)+2;
...
PlotP1P2LinesMid(i,l1line,l2line,exl1,exl2,Hi2Day,Low1Day, ...
    DiagL1,DiagL2,exDiag1,exDiag2,DLinesL1,DLinesL2, ...
    HLinesL1,HLinesL2,MLines,BLines,oBox,0.5*spread,i,1,1,av(i),bv(i))
[a,b]=GetProfit(mids);
pr1=sum(a); pr2=sum(b);
[a,b]=GetProfit(boxes);
pb1=sum(a); pb2=sum(b);
