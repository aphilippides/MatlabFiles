...
function DynSysAnalysis_Answers(StartX,NumIters,ExtInput,C1,StartY,C2)

%ExpGrowth(StartX,ExtInput,C1,NumIters)
...
% from time t to t+tstep starting from StartX.
% k is the growth constant
function ExpGrowth(StartX,tstep,k,MaxT)

% Initialise a vector to hold x values
x(1)=StartX;
for t=1:MaxT/tstep
    % Calculate the next x value and put it in a vector
    x(t+1)= x(t) + tstep*k*x(t);
end
ts=[0:tstep:MaxT];
plot(ts,x)