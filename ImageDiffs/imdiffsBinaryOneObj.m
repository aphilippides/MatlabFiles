function[dnew]=imdiffsBinary(goal,Lims,skyc)
...
if(nargin<3) 
    skyc=250; 
    outfile=['Goal' int2str(goal) 'Med' int2str(Lims) 'Bin1Obj.mat'];
else outfile=['Goal' int2str(goal) 'SkyC' int2str(skyc) 'Bin1Obj.mat'];
end;
fn=s(goal).name;
load(fn);
...
[g_nosky,g_noskyhe,g_bin,g_sh,g_sl,g_sum,g_sumh,g_sumb]=binim(gim,g_he,newimall,Lims,skyc,s(goal).name);
...
for j=1:1:length(s)
    ...
    newim=double(newim);
    [nosky,noskyhe,bin,sh,sl,sums,sumh,sumb,va]=binim(newim,he_im,newimall,Lims,skyc,s(j).name);
    skyhi(j,:)=sh;
    skylo(j,:)=sl;
    sky_sum(j,:)=sums;
    sky_sumh(j,:)=sumh;
...
%         figure(1)
% subplot(3,1,1+(j-1)/5)
        imagesc(noskyhe), hold on, plot(sh,'r','LineWidth',1.5), hold off, axis image
        colormap gray
        Setbox;
%         figure(2),
...
function[il,ilhe,bina,skyl_hi,skyl_lo,skysum,skysumh,skysumb,vals]=binim(newim,imhe,imrgb,Lims,skyc,fn);
...
S=regionprops(bl,'Area','PixelList');
[m,maxa]=max([S.Area]);
for i=1:length(S)
    if(i~=maxa)
...
function[il,ilhe,bina,skyl_hi,skyl_lo]=binimAll(newim,imhe,imrgb,Lims,skyc,fn);
