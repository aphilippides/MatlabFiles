function[dnew]=imdiffsBlurBin(goal,Lims,skyc)

s=dir('I*_BlurBinary.mat')

if(nargin<2) Lims=1; end;
if(nargin<1) goal=length(s);end;
if(nargin<3) 
    skyc=250; 
%     outfile=['Goal' int2str(goal) 'Med' int2str(Lims) 'BlurBin1Obj.mat'];
% else
end;
if(Lims==1) outfile=['Goal' int2str(goal) 'SkyC' int2str(skyc) 'BlurBin1Obj.mat'];
else outfile=['Goal' int2str(goal) 'SkyC' int2str(skyc) 'BlurBin.mat'];
end
...
if(Lims==1) [g_nosky,g_noskyhe,g_bin,g_sh,g_sl,g_sum,g_sumh,g_sumb]=binim(gim,g_he,bina,newimall,Lims,skyc,s(goal).name);
else [g_nosky,g_noskyhe,g_bin,g_sh,g_sl,g_sum,g_sumh,g_sumb]=binimAll(gim,g_he,bina,newimall,Lims,skyc,s(goal).name);
end
...
for j=1:length(s)
    ...
    newim=double(newim);
    if(Lims==1) [nosky,noskyhe,bin,sh,sl,sums,sumh,sumb,va]=binim(newim,he_im,bina,newimall,Lims,skyc,s(j).name);
    else [nosky,noskyhe,bin,sh,sl,sums,sumh,sumb,va]=binimAll(newim,he_im,bina,newimall,Lims,skyc,s(j).name);
    end
    skyhi(j,:)=sh;
    skylo(j,:)=sl;
    sky_sum(j,:)=sums;
    ...
    [min_ns(j),rim3,imin3,mind3(j),dd3(j,:)]=VisualCompass(nosky,g_nosky);
...
%    plot([g_sum;g_sumh;g_sumb]','r'),hold on,plot([sums;sumh;sumb]','b'),hold off
end
...
for i=1:(size(ang_all,1))
    [es1,ns]=min([ang_all(i,:)-1;ww-ang_all(i,:)]);
...
function[il,ilhe,bina,skyl_hi,skyl_lo,skysum,skysumh,skysumb,vals]=binim(newim,imhe,bina,imrgb,Lims,skyc,fn);
...
function[il,ilhe,bina,skyl_hi,skyl_lo]=binimAll(newim,imhe,bina,imrgb,Lims,skyc,fn);
...
il=double(newim).*double(bina)+skyc*double(~bina);
ilhe=double(imhe).*double(bina)+skyc*double(~bina);
