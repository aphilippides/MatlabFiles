...
for Gen=1:NumGens
   for i=1:POPSIZE
      ...
      Fit=TestRbfX(Ch,TrainS,TestS,Thresh,Equal);
      Ch(end)=min(Fit,TestRbf(Ch,TrainS,TestS,Thresh,Equal));
...
if(Gen==0)
   ...
   XBest=TestRbfX(Pop(1,:),TrainS,TestS,Thresh,Equal);
   dummy=[Inds MeanFs(end) BestFs(end) WorstFs(end) StdFs(end) TestBest XBest]
	save(FileN,'Inds','MeanFs','BestFs','WorstFs','StdFs','BestInds','TestBest','XBest','Pop','Thresh','Equal');
elseif(mod(PopNum,DUMPEVERY)==0)
   load(FileN,'MeanFs','BestFs','WorstFs','StdFs','BestInds','TestBest','XBest');
   ...
   XF=TestRbfX(Pop(1,:),TrainS,TestS,Thresh,Equal);
   TestBest=[TestBest TestF];
   XBest=[XBest XF];
	dummy=[Gen MeanFs(end) BestFs(end) WorstFs(end) StdFs(end) TestF XF]
   save(FileN,'Inds','MeanFs','BestFs','WorstFs','StdFs','BestInds','Pop','TestBest','XBest','Thresh','Equal');
end
...
for i=1:size(Pop,1)
   Fit=TestRbfX(Pop(i,:),TrainS,TestS,Thresh,Equal);
   Pop(i,end)=min(Fit,TestRbfX(Pop(i,:),TrainS,TestS,Thresh,Equal));
end
NewPop=SortPop(Pop);
