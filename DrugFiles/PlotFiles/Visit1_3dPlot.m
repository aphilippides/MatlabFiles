...
set(h,'Position',[Pos(1) Pos(2)+.17 Pos(3)])
...
set(h,'Position',[Pos(1) Pos(2)+.25 Pos(3)])
...
eval(['print -djpeg100 Visit13dSc' int2str(Scale) 'LHNot4.jpg']);
...
for i=1:N
   ...
   x=length(TextStr)/2;
   text(i/(N+1)-x*.015,-.25,TextStr,'Units','normalized','FontSize',FSize);   
%   text(i/(N+1)-.025 -i*.005,-.25,TextStr,'Units','normalized','FontSize',FSize);   
end
text(-.075,-.1,'Gamma =','Units','normalized','FontSize',FSize);   
text(-.0,-.25,'C =','Units','normalized','FontSize',FSize);   
