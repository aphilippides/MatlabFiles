function DrugAllGA_Min(TrainS,ValS,TestS,Thresh,Equal,SaveFName,Scale)

XRate=0.2;
MRate=0.01;
NumGens=50;
global POPSIZE DUMPEVERY CMAX CMIN MINGAM MAXGAM SCALES;
global SDMUT MEANC SDC MEANGAM SDGAM IndLen;
POPSIZE=50;
DUMPEVERY=10;
...
SDMUT=5;
IndLen=size(TrainS,2)+2;  % Gamma, C, Scale, 216 Chems, Fitness
Pop=zeros(POPSIZE,IndLen);
Pop=GetInitialPop(Pop,0.5,TrainS,ValS,Thresh,Equal);
OutputPopStats(Pop,0,0,SaveFName,TrainS,ValS,TestS,Thresh,Equal);

for Gen=1:NumGens
   for i=1:POPSIZE
      Ch=CreateChild(Pop,XRate,MRate);
      XScore=TestRbfX(Ch,TrainS,ValS,Thresh,Equal);
      TScore=TestRbf(Ch,TrainS,ValS,Thresh,Equal);
      Ch(end)=min(TScore,XScore);
      Pop=UpdatePop(Pop,Ch);
      OutputPopStats(Pop,Gen,i,SaveFName,TrainS,ValS,TestS,Thresh,Equal);  
	end
end
...
function OutputPopStats(Pop,Gen,PopNum,FileN,TrainS,ValS,TestS,Thresh,Equal)
...
if(Gen==0)
   ...
   StdFs=std(Pop(:,end));
   BestInds=Pop(1,:);
   ValBest=TestRbf(Pop(1,:),TrainS,ValS,Thresh,Equal);
   TestBest=TestRbf(Pop(1,:),TrainS,TestS,Thresh,Equal);
   dummy=[Inds MeanFs(end) BestFs(end) WorstFs(end) StdFs(end) TestBest ValBest]
	save(FileN,'Inds','MeanFs','BestFs','WorstFs','StdFs','BestInds','TestBest','Pop','Thresh','Equal','ValBest');
elseif(mod(PopNum,DUMPEVERY)==0)
   load(FileN,'MeanFs','BestFs','WorstFs','StdFs','BestInds','TestBest','ValBest');
   ...
   ValF=TestRbf(Pop(1,:),TrainS,ValS,Thresh,Equal);
   ValBest=[ValBest ValF];
	TestF=TestRbf(Pop(1,:),TrainS,TestS,Thresh,Equal);
   TestBest=[TestBest TestF];
	dummy=[Gen MeanFs(end) BestFs(end) WorstFs(end) StdFs(end) TestF ValF]
   save(FileN,'Inds','MeanFs','BestFs','WorstFs','StdFs','BestInds','Pop','TestBest','Thresh','Equal','ValBest');
end
...
for i=1:size(Pop,1)
   XScore=TestRbfX(Pop(i,:),TrainS,TestS,Thresh,Equal);
   TScore=TestRbf(Pop(i,:),TrainS,TestS,Thresh,Equal);
   Pop(i,end)=0.25*max(TScore,XScore)+0.75*min(TScore,XScore);
end
NewPop=SortPop(Pop);
