function DrugAllGA_X2(TrainS,ValS,TestS,Thresh,Equal,SaveFName,Scale)
...
for Gen=1:NumGens
   for i=1:POPSIZE
      ...
      Ch(end)=0.25*max(TScore,XScore)+0.75*min(TScore,XScore);
...
elseif(mod(PopNum,DUMPEVERY)==0)
   ...
   TestBest=[TestBest TestF];
	dummy=[Gen MeanFs(end) BestFs(end) WorstFs(end) StdFs(end) TestF ValBest]
