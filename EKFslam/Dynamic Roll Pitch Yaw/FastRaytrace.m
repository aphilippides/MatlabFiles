...
Xant=[ant.x;ant.y;ant.z;ant.yaw;ant.pitch;ant.roll;ant.dx;ant.dy;ant.dz;ant.dyaw;ant.dpitch;ant.droll];
...
min_dist_to_heading = 0.25;
Err=sqrt(sum((ant.vert-ones(size(ant.vert,1),1)*h').^2,2));
...
for i=1:n_facets
    ...
    if ((Dx < maxx) & (Dx > minx) & (Dy < maxy) & (Dy > miny) & (Dz < maxz) & (Dz > minz)) 
        ...
        X=X+ant.vert(ind(i),1);
        Y=Y+ant.vert(ind(i),2);
        Z=Z+ant.vert(ind(i),3);
    else
