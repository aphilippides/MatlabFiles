...
if n_mL>0
    cols=[1,2,3,2,3,3,3,3,3,3,3,3,2,3,3,3,3,3,3,3,3,2,3,3,3,3,3,3,3,3,2,3,3,3,3,3,3,3];
    for k=1:n_mL
        x=20*rand(1);
        if rand(1)>0.5
            x=-x;
...
if n_lm>0
    for i=1:n_lm
        L(i+j+k,:)=[(2*world_sz*(rand(1,2)-0.5)),0.1,0.1,(world_sz/10)*rand(1),3];
    end
else
    i=k;
end
