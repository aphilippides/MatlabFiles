...
while(1)
    ...
    if(~isempty(t_ind)&&(Sequential==1))
        % adjust the indices sequentially
        % here we don't care whether something has been adjusted or not
        i=t_ind(1);
        TStr=['Time' num2str(t(i))];
        t_ind=setdiff(t_ind,i);
        
        % this bit might not be necessary - can all be controlled by
        % t_ind's emptiness or not, but could be handy  for flexibiity
        if(isempty(t_ind))
            Sequential=0;
        end
    else
...
%         figure(2); plot(t(NotDone),e,t(i),e(ind),'go'),
    ...
    % Get the trace and midpoint for plotting before bees in one colour and
    % after in another colour
    tri=max(1,i-TrL):min(maxl,i+TrL);
    mp=find(tri==i);
    if(length(tri)<2) 
        tr=[];
    ...
    if(sk==1)
        % if sk=1 is picked n was pressed in AdjustOrientsExpt2 so go on to
        % the next criterion
        nx=nx+1;
    elseif(sk==2)
        % if sk=2 is picked t was pressed in AdjustOrientsExpt2 so adjust
        % some points based on their times
        Sequential=1;
        Ts=ForceNumericInput('enter times to flip as a vector [t1 t2]: ',1);
        
        % get the indices of the times picked 
        t_ind=GetTimes(t,Ts);
        
        % output a warning if no data
        if(isempty(t_ind))
            disp(' ');
            disp(['no data between times [' num2str(Ts) ']']);
            Sequential=0;
        end
    else
        if((b==0)&&(u==0))
