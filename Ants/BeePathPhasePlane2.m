...
for i=1:length(Picked)
    ...
    a1=AngleWithoutFlip(sOr_sc);
    a2=AngleWithoutFlip(Cent_Os);
    da2=Medianfilt(MyGradient(a2,t),5);
    da1=Medianfilt(MyGradient(a1,t),5);
...
%                 end
                ...
                if(nlm>1)
                    ...
                    subplot(3,2,4)
                    plot(t(i2),sOr_sc(i2),'r',t(i2),Cent_Os(i2),'b')
                    xlabel('body o (red) vs flight dir (blue)')
                end
                subplot(3,2,5),
                plot(Cent_Os(i2),sOr_sc(i2),'r- .',Cent_Os(is),sOr_sc(is),'b- .','MarkerSize',msize)%,nor(inps),fdif(inps),'bo')%,axis equal
                xlabel('body o (y-axis) and flight dir (x-axis)')
                subplot(3,2,6),
                plot(da2(i2),da1(i2),'r- .',da2(is),da1(is),'b- .','MarkerSize',msize)%,nor(inps),fdif(inps),'bo')%,axis equal
                xlabel('d(body)/dt (y-axis) and d(flight)/dt (x-axis)')
