...
if(isempty(Picked)) 
    if(ino)
        fst=input('Enter new filename start *ZigZagData.mat:  ','s');
        OutFn=[fst 'ZigZagData.mat'];
        zigzag=[];
    else
        fst=input('Enter new filename start *ZZLoopData.mat:  ','s');
        OutFn=[fst 'ZZLoopData.mat'];
        loops=[];
    end;    
else
...
for j=1:length(fn)
    ...
    if(do)
        if(ino)
            zz=GetZigZag(fn(j).name,athresh,NestArc);
            zigzag=[zigzag zz];
            save(OutFn,'zigzag');
            CheckZZ(zigzag,OutFn);
        else
            zz=GetLoop(fn(j).name,athresh,NestArc);
            loops=[loops zz];            
            save(OutFn,'loops');
            CheckLoops(loops,OutFn);
        end
    end
end
