function[NumBees,Cents,Areas,Orients,Eccents,EndPt,MajAx,MinAx,bounds]=Find1Bee_HiSpeed(refim,im)
Cents=[];Areas=[];Orients=[];Eccents=[];EndPt=[];MajAx=[];MinAx=[];bounds=[];
d=refim(:,:,1)-im(:,:,1);
...
if(n>=1)
    ...
    for j=1:NumBees
        ...
        % Orients(j)=GetThinEnd(beeim,Orients(j),Cents(j,:),MajAx(j),4);
        % Orients(j)=GetThinEnd((im(:,:,1)),Orients(j),);
        [EndPt(j,1) EndPt(j,2)]=pol2cart(Orients(j),S(j).MajorAxisLength/2);
        bounds=[bounds;a];
    end
    EndPt=EndPt+Cents;
