function varargout = VidEditorIndoor(varargin)
% VIDEDITORINDOOR M-file for VidEditorIndoor.fig
%      VIDEDITORINDOOR, by itself, creates a new VIDEDITORINDOOR or raises the existing
%      singleton*.
%
%      H = VIDEDITORINDOOR returns the handle to a new VIDEDITORINDOOR or the handle to
%      the existing singleton*.
%
%      VIDEDITORINDOOR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in VIDEDITORINDOOR.M with the given input arguments.
%
%      VIDEDITORINDOOR('Property','Value',...) creates a new VIDEDITORINDOOR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before VidEditorIndoor_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to VidEditorIndoor_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help VidEditorIndoor
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @VidEditorIndoor_OpeningFcn, ...
    'gui_OutputFcn',  @VidEditorIndoor_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before VidEditorIndoor is made visible.
function VidEditorIndoor_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to VidEditorIndoor (see VARARGIN)

% Choose default command line output for VidEditorIndoor
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes VidEditorIndoor wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = VidEditorIndoor_OutputFcn(hObject, eventdata, handles)
