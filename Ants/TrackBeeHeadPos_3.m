function[bads] = TrackBeeHeadPos_3(s,ns)
% dwork;
...
fn=['HeadPos' int2str(s) 'Fr' int2str(ns(1)) '_' int2str(ns(end)) 'Hand_3.mat'];
...
for i=ns
    ...
    if(n==1)
        ...
        while(redo)
            ...
            title(['Click each antenna base only. Fr ' int2str(i) '/' int2str(m)])
            ...
            elseif(ismember(115,b)|(length(p)~=inps))
                ...
                if(~redo)
                    if(~skip)
                        AntennaeBase=[AntennaeBase; antennae];
