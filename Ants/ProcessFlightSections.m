...
for i=1:length(fltsec)
    ...
    for j=1:length(fltsec(i).fsec);
        ...
        if(size(cs,1)>=4)
            ...
            [br,stats]=robustfit(cs(:,1),cs(:,2));
            codat.cs=cs';
            codat.br=br;
            codat.rmse=stats.ols_s;
            brs=[brs br(2)];
            % check the robst fit residual errors
            a=atan2(br(2),1);
            ...
            codat.dco=AngularDifference(fltsec(i).fsec(j).co,MeanAngle(fltsec(i).fsec(j).co))';
            codat.mdco=max(abs(codat.dco));
            codat.aco=AngleWithoutFlip(fltsec(i).fsec(j).co)';
            codat.ran=max(codat.aco)-min(codat.aco);
...
odat.atn=atn;
odat.atl=atl;
save(['CoRangeDat' fstr '.mat'],'corangeDat')
