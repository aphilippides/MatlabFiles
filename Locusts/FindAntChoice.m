function[Areas,Cents,Orients,meancol,EndPt,NumBees,bounds,NLess ...
    area_e,EPt,ang_e,ellips,len_e,eccent,outlines]=...
    FindAntChoice(im,odeven,sm_opt,thresh,bigsmall,mask,opt)

% sets the default size to remove big or small objects
% kind of defunct as should always be set so I've commented it out
% if(nargin<5)
%     bigsmall=[400 20];
% end
...
% smooth with a gaussian filter of width opt
% should really erode the mask to get rid of edges but should be fine 
if(sm_opt(1)>0)
    h=fspecial('gaussian',sm_opt(1),sm_opt(2));
...
% now exclude anything outside the mask
e=e.*mask;
...
S = regionprops(L,'Area','Centroid','Orientation','MajorAxisLength','MinorAxisLength','Eccentricity','BoundingBox','PixelList','Extrema');
% ProperBees=[];
% if(NumBees>3)
%     keyboard;
% end
if(NumBees>0)
    for i=1:NumBees
        ...
        newim=double(im(r1:r2,c1:c2)).*double(e(r1:r2,c1:c2));
        nim=newim(:);
        nim=nim(nim>0);
