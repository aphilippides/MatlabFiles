function Analyse_Data_Facet(FileName)
...
DataFileName2 = strcat(FileName, '_Facet_Monitor') ;
load(DataFileName2);
% LOADED :
%   mx_Facet       my_Facet      HomewardComp_Facet       FacetNumSteps
...
%                   obj,nest,fac_awidth,SideEffectWidth,NObj,mx_Facet,my_Facet, ...
%                   HomewardComp_Facet,FacetNumSteps,trapx_Alv,trapy_Alv,0) 
% ;
% 
% figure(34)
% hold off
...
% axis([X_Alv(1)  X_Alv(end)  Y_Alv(1)  Y_Alv(end)]) ;
% hold on
% pcolor(X_Alv,Y_Alv,FacetNumSteps) ;
% shading flat;
% hold off
% 
% tmp=~isnan(AlvNumSteps)-~isnan(FacetNumSteps);
% tmp(1)=1;
% tmp(2)=-1;
% 
% figure(39)
...
Catch_Size_Facet = Get_Catch_Size(FacetNumSteps,X_Alv(1,2)-X_Alv(1,1),Y_Alv(2,1)-Y_Alv(1,1));
Catch_Env_Ratio_Facet=Catch_Size_Facet/(EnvSize - 2*SideEffectWidth)^2 ;

File_Analysis=strcat(FileName,'_Facet_Analyse.mat');
if (exist(File_Analysis))
    save( File_Analysis,...
                'Catch_Env_Ratio_Facet' ,...
           '-append');
else
    save( File_Analysis,...
                'Catch_Env_Ratio_Facet');
