function Analyse_Data_Edge(FileName)

if(nargin<1)
    FileName = 'C:\Temp\Desmottes\Env_35Obj_36' ;
end
...
DataFileName2 = strcat(FileName, '_Edge_Monitor') ;
load(DataFileName2);
% LOADED :
%   mx_Edge       my_Edge      HomewardComp_Edge       EdgeNumSteps
...
%                   obj,nest,fac_awidth,SideEffectWidth,NObj,mx_Edge,my_Edge, ...
%                   HomewardComp_Edge,EdgeNumSteps,trapx_Alv,trapy_Alv,0) 
% ;
% 
% figure(34)
% hold off
...
% axis([X_Alv(1)  X_Alv(end)  Y_Alv(1)  Y_Alv(end)]) ;
% hold on
% pcolor(X_Alv,Y_Alv,EdgeNumSteps) ;
% shading flat;
% hold off
% 
% tmp=~isnan(AlvNumSteps)-~isnan(EdgeNumSteps);
% tmp(1)=1;
% tmp(2)=-1;
% 
% figure(39)
...
Catch_Size_Edge = Get_Catch_Size(EdgeNumSteps,X_Alv(1,2)-X_Alv(1,1),Y_Alv(2,1)-Y_Alv(1,1));
Catch_Env_Ratio_Edge=Catch_Size_Edge/(EnvSize - 2*SideEffectWidth)^2 ;

File_Analysis=strcat(FileName,'_Edge_Analyse.mat');
if (exist(File_Analysis))
    save( File_Analysis,...
                'Catch_Env_Ratio_Edge' ,...
           '-append');
else
    save( File_Analysis,...
                'Catch_Env_Ratio_Edge');
