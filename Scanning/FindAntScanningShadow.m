function[Areas,Cents,Orients,meancol,EndPt,NumBees,bounds,NLess, ...
    area_e,EPt,ang_e,ellips,len_e,eccent]=FindAntScanningShadow(im,t,SLev,threshval,plo)
...
% fill in holes
bwclean=bwareaopen(bwclean,(tlil-10),8);
...
if(nargin<3) plo=0; end;
if(plo)
    subplot(2,2,1)
    imagesc(origim)
    colormap('gray');xlabel('original')
    subplot(2,2,2)
    imagesc(d)
    xlabel('difference image')
    subplot(2,2,3)
    imagesc(e)
    xlabel('threshold difference')
    subplot(2,2,4)
    imagesc(L)   
    xlabel('threshold and shadow')
    return
end
NLess=[];meancol=[];
% ProperBees=[];
