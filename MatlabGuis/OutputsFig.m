% figure window for showing gasnet outputs

function OutputsFig

% Define initial constants,Read in data and get needed constants

EdHt = 0.05;
Gap=0.025;
OutData = LoadDataFile('NetActivity');
%OutData = LoadDataFile('Activ');
NumNodes = OutData(1,1);
[tmp,n] =size(OutData);
Iters=tmp-2;
...
edStarth=uicontrol('style','edit', 'Units', 'normalized',...
   'position', [0.6 0.95 0.1 EdHt], ...
   'Tag', 'edStart', ...
   'callback', 'OutputsFigCode SetAxes', ...
   'String',int2str(1));

edPlotLenh=uicontrol('style','edit', 'Units', 'normalized',...
   'position', [0.4 0.95 0.1 EdHt], ...
   'Tag', 'edPlotLen', ...
   'callback', 'OutputsFigCode SetAxes', ...
   'String',int2str(Iters), ...
   'Userdata',Iters);

NodeList = 0:1:NumNodes-1;
edNodesh=uicontrol('style','edit', 'Units', 'normalized',...
   'position', [0.05 0.95 0.3 EdHt], ...
   'Tag', 'edNodes', ...
   'callback', 'OutputsFigCode PlotOutputs', ...
   'String',int2str(NodeList));

edTransIth=uicontrol('style','edit', 'Units', 'normalized',...
   'position', [0.85 0.95 0.1 EdHt], ...
   'Tag', 'edTransIt', ...
   'callback', 'OutputsFigCode SetTransNum', ...
   'String',int2str(1), ...
   'Userdata',Iters);
OutputsFigCode PlotOutputs;

% Buttons 

butTransIth=uicontrol('style','pushbutton', 'Units','normalized',...
   'position', [ 0.95 0.95 0.05 EdHt ], ...
   'Tag', 'butTransIt', ...
   'callback', 'OutputsFigCode TransIt', ...
   'String','Iter');

butOneIth=uicontrol('style','pushbutton', 'Units','normalized',...
   'position', [ 0.0 0.8 0.05 EdHt ], ...
   'Tag', 'butOneIt', ...
   'callback', 'OutputsFigCode OneIt', ...
   'String','OneIt',...
   'Userdata',OutData(2,:));
