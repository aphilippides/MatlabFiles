...
Space=10;
Sq=2;
h=subplot('Position',[0.175 0.785 0.45 0.2]);
XT=1:10;
NumTs=XT.^2;
DataNeeded=0;
if(DataNeeded==1)
...
plot(NumTs,AffVol./SynthArea,[Col '- .']);
...
subplot('Position',[ 0.725 0.725 0.3 0.3 ])
X=25;
Time=500;
DrawLobeOutline(X,Sq,Space,Time,Col);

FigHdl=gcf;
set(FigHdl,'Units','centimeters');
X=get(FigHdl,'Position');
set(FigHdl,'Position',[X(1) X(2) 8.8 10]);
...
h=subplot('Position',[0.825 StartY2 0.035 FigHt]);
colorbar(h)
XLim=get(h,'YLim');
...
h=subplot('Position',[0.825 StartY 0.035 FigHt]);
colorbar(h)
SetYTicks(h,0,1e6,2,XTPos,Labs);
set(h,'TickLength',[0 0],'FontSize',8,'Box','off')
ylabel('Concentration (\muM)')
...
function DrawLobeOutline(XS,Sq,Sp,T,Col)

x1=400;
x2=600;
DataNeeded=0;
dmat
fname=['MeshPaperFiles/OpticLobe/Fig4OutLineData.mat'];
...
XRow=sqrt(XS);
DrawSources(left,right,top,bott,Sp,XRow,Col)
...
function DrawSources(left,right,top,bott,Sp,X,Col)
...
for i=1:length(Xes)
   Xs=ones(1,X)*Xes(i);
   h=plot(Xs,Ys,[Col 'o']);
   set(h,'MarkerSize',3,'MarkerFaceColor',Col)
end
