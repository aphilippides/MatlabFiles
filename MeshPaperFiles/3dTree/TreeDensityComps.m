function TreeDensityComps(MStr,GridSize, Density,Slice,Version)

Sqare=1;
colstr='g'
% Get data
filename=[MStr 'TreeRho' num2str(Density) '/TreeV' int2str(Version) 'MaxGr' num2str(GridSize) 'Sq' num2str(Sqare) 'Sl' num2str(Slice) '.dat']
filename2=[MStr 'TreeRho' num2str(Density) '/TreeV' int2str(Version) 'BinsGr' num2str(GridSize) 'Sq' num2str(Sqare) 'Sl' num2str(Slice) '.dat']
M=load(filename);
M2=load(filename2);
[X2,Y2]=size(M2);
[X,Y]=size(M);
